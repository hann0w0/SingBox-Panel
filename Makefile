...
panel:
	go build -ldflags "$(LDFLAGS)" -o $(DIST)/singpanel ./cmd/panel

## cross-compile the agent for the common Linux VPS architectures
agents:
	mkdir -p $(DIST)/agents
	GOOS=linux GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -o $(DIST)/agents/singpanel-agent-linux-amd64 ./cmd/agent
	GOOS=linux GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -o $(DIST)/agents/singpanel-agent-linux-arm64 ./cmd/agent
...
run: panel agents
	./$(DIST)/singpanel --config ./deploy/panel.example.yaml
