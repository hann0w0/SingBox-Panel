...
import (
	...
	"github.com/hann0w0/singbox-panel/internal/services/agent"
)
...
func main() {
	...
	flagToken := strings.TrimSpace(*tokenFlag)
	envToken := strings.TrimSpace(os.Getenv("SINGBOX_PANEL_TOKEN"))
	token := firstNonEmpty(flagToken, envToken, fileConf["TOKEN"])
