...
func (a *App) listOutbounds(c *gin.Context) {
	...
	a.db.Where("server_id = ?", id).Order("sort").Find(&rows)
...
// testEgress measures the node's own latency to the public internet. It is a
// property of the node (not of any single inbound), so every inbound row on the
// page reports the same figure.
func (a *App) testEgress(c *gin.Context) {
	sid, ok := uintParam(c, "id")
	if !ok {
		return
	}
	ctx, cancel := context.WithTimeout(c.Request.Context(), 20*time.Second)
	defer cancel()
	res, err := a.hub.SendCommand(ctx, sid, protocol.CmdTestEgress, protocol.TestEgressCmd{})
	if err != nil {
		if strings.Contains(err.Error(), "unknown command") {
			c.JSON(http.StatusBadRequest, gin.H{"error": "该节点的 Agent 版本过旧，请先点「升级 Agent」"})
			return
	...
	var data protocol.TestEgressData
	if err := json.Unmarshal(res.Data, &data); err != nil {
		c.JSON(http.StatusBadGateway, gin.H{"error": "解析测试结果失败"})
		return
	}
	c.JSON(http.StatusOK, gin.H{
		"ok": data.OK, "latency_ms": data.LatencyMS, "status": data.Status,
		"error": data.Error, "target": protocol.DefaultEgressURL,
	})
}
...
func (a *App) listRuleSets(c *gin.Context) {
	...
	a.db.Where("server_id = ?", id).Order("id").Find(&rows)
