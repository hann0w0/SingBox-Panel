...
func groupedServerCounts(db *gorm.DB, modelValue any) (map[uint]int64, error) {
	var rows []perServerCount
...
func (a *App) adminOverview(c *gin.Context) {
	...
	if err := a.db.Model(&model.User{}).Where("role = ?", model.RoleUser).Count(&users).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	if err := a.db.Model(&model.Inbound{}).Count(&inbounds).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	if err := a.db.Model(&model.Outbound{}).Count(&outbounds).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	if err := a.db.Model(&model.RouteRule{}).Count(&rules).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	var usersEnabled, usersExpiring int64
	if err := a.db.Model(&model.User{}).Where("role = ? AND enabled = ?", model.RoleUser, true).Count(&usersEnabled).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	if err := a.db.Model(&model.User{}).
		...
		Count(&usersExpiring).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	if err := a.db.Order("id").Find(&servers).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	applyServerOrder(a.db, servers)
	inboundCounts, err := groupedServerCounts(a.db, &model.Inbound{})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	outboundCounts, err := groupedServerCounts(a.db, &model.Outbound{})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	ruleCounts, err := groupedServerCounts(a.db, &model.RouteRule{})
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	for i := range servers {
		...
		nodes = append(nodes, nodeBrief{
			...
			Inbounds: int(inboundCounts[srv.ID]), Outbounds: int(outboundCounts[srv.ID]), Rules: int(ruleCounts[srv.ID]),
		})
	}
...
func (a *App) listServers(c *gin.Context) {
	...
	if err := a.db.Preload("Inbounds").Order("id").Find(&servers).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	applyServerOrder(a.db, servers)
...
func (a *App) createServer(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"server":          srv,
		"install_command": a.newAgentInstallCommand(srv.ID),
...
func (a *App) deleteServer(c *gin.Context) {
	...
	unlockConfig := func() {}
	if a.orch != nil {
		unlockConfig = a.orch.lockServer(id)
	}
	defer unlockConfig()
	a.userAccessMu.Lock()
	...
	unlockLogs := a.logSessionLocks.lock(id)
	a.logSessionsMu.Lock()
	for _, lease := range a.logSessions[id] {
		if lease.Timer != nil {
			lease.Timer.Stop()
		}
	}
	delete(a.logSessions, id)
...
// uninstallSingbox removes the remote sing-box package, binary and systemd
// unit while preserving its configuration, Agent and panel-side node data.
func (a *App) uninstallSingbox(c *gin.Context) {
	id, ok := uintParam(c, "id")
	if !ok {
		return
	}
	unlockOperation := a.lockServerOperation(id)
	defer unlockOperation()
	ctx, cancel := context.WithTimeout(c.Request.Context(), 2*time.Minute+30*time.Second)
	defer cancel()
	res, err := a.hub.SendCommand(ctx, id, protocol.CmdUninstallSingbox, nil)
	if err != nil {
		...
		case errors.Is(err, ErrAgentOffline):
			...
			message = "节点离线，无法卸载 Sing-box"
		case strings.Contains(err.Error(), "unknown command"):
			status = http.StatusBadRequest
			message = "该节点的 Agent 版本过旧，请先点击「安装 / 升级 Agent」完成自动升级后再卸载 Sing-box"
		}
		c.JSON(status, gin.H{"error": message, "output": res.Output})
		return
	}
	if err := a.db.Model(&model.Server{}).Where("id = ?", id).Updates(map[string]any{
		"singbox_installed": false,
		"singbox_version":   "",
		"singbox_active":    false,
	}).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error(), "output": res.Output})
		return
	}
	c.JSON(http.StatusOK, gin.H{"ok": true, "output": res.Output})
}
...
func (a *App) uninstallAgent(c *gin.Context) {
	...
	a.invalidateAgentInstallCodes(id)
	c.JSON(http.StatusOK, gin.H{"ok": true, "output": res.Output})
}
...
// Version direction is intentionally ignored: an operator-triggered sync must
// also recover from release-number resets. The Agent validates the downloaded
// binary and compares its SHA-256 before staging any replacement.
func (a *App) updateAgent(c *gin.Context) {
	...
	defer cancel()
	res, err := a.sendAgentUpdateCommand(ctx, id)
	if err != nil {
		...
		if strings.Contains(err.Error(), "unknown command") {
			c.JSON(http.StatusBadRequest, gin.H{
				"error": "该节点的 Agent 版本过旧，不支持一键升级。请在这台服务器上重跑一次安装命令（升级后本按钮即可使用）：\n" +
					a.newAgentInstallCommand(srv.ID),
			})
			return
		}
		c.JSON(http.StatusBadGateway, gin.H{"error": err.Error(), "output": res.Output})
	...
	c.JSON(http.StatusOK, gin.H{
		"ok":      true,
		"updated": agentUpdatePerformed(res.Output),
		"output":  res.Output,
	})
}

func agentUpdatePerformed(output string) bool {
	return !strings.HasPrefix(strings.TrimSpace(output), "Agent 已是最新版本")
}

func (a *App) sendAgentUpdateCommand(ctx context.Context, serverID uint) (protocol.CommandResultEvt, error) {
	if a.hub == nil {
		return protocol.CommandResultEvt{}, ErrAgentOffline
	}
	connection := a.hub.connection(serverID)
	if connection == nil {
		return protocol.CommandResultEvt{}, ErrAgentOffline
	}
	return a.hub.sendCommandToConnection(ctx, connection, protocol.CmdUpdateAgent, nil)
}
...
func (a *App) updateAllAgents(c *gin.Context) {
	if a.hub == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "agent hub unavailable"})
		return
	}
	serverIDs := a.hub.OnlineServerIDs()
	sort.Slice(serverIDs, func(i, j int) bool { return serverIDs[i] < serverIDs[j] })
	if len(serverIDs) == 0 {
		c.JSON(http.StatusOK, gin.H{
			"ok": true, "message": "当前没有在线的服务器", "count": 0,
			"requested": 0, "succeeded": 0, "failed": 0, "results": []agentUpdateResult{},
		})
		return
	}

	var servers []model.Server
	if err := a.db.Where("id IN ?", serverIDs).Find(&servers).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	serversByID := make(map[uint]model.Server, len(servers))
	for i := range servers {
		serversByID[servers[i].ID] = servers[i]
	}
	results := make([]agentUpdateResult, len(serverIDs))
	jobs := make(chan int, len(serverIDs))
	...
	for range workers {
		go func() {
			...
			for index := range jobs {
				...
				server := serversByID[serverID]
				result := agentUpdateResult{ServerID: serverID, ServerName: server.Name}
				if result.ServerName == "" {
					result.Error = "服务器记录不存在"
					results[index] = result
					continue
				}
				unlockOperation := a.lockServerOperation(serverID)
				res, err := a.sendAgentUpdateCommand(ctx, serverID)
				unlockOperation()
				result.Output = trimRunes(res.Output, 2000)
				if err != nil {
					result.Error = trimRunes(strings.TrimSpace(err.Error()), 500)
					if result.Error == "" {
						result.Error = "Agent 更新失败"
					}
				} else {
					result.Success = true
				}
	...
	message := fmt.Sprintf("Agent 同步完成：%d 台成功，%d 台失败", succeeded, failed)
...
func (a *App) listInbounds(c *gin.Context) {
	...
	if err := a.db.Where("server_id = ?", id).Order("id").Find(&inbounds).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (a *App) deleteInbound(c *gin.Context) {
	...
	err = a.db.Transaction(func(tx *gorm.DB) error {
		if err := tx.Delete(&ib).Error; err != nil {
			return err
		}
		if err := deleteUserNodeOrderRefs(tx, userNodeTypeManaged, []uint{ib.ID}); err != nil {
			return err
		}
		return removeInboundUserAssignments(tx, ib.ID)
	})
	a.userAccessMu.Unlock()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// removeInboundUserAssignments removes an explicitly assigned inbound without
// turning an empty explicit list into an accidental whole-server grant.
func removeInboundUserAssignments(tx *gorm.DB, inboundID uint) error {
	var users []model.User
	if err := tx.Find(&users).Error; err != nil {
		return err
	}
	for i := range users {
		if !containsID(users[i].InboundIDs, inboundID) {
			continue
		}
		remaining := removeID(users[i].InboundIDs, inboundID)
		var inbounds []model.Inbound
		if len(remaining) > 0 {
			if err := tx.Where("id IN ?", remaining).Find(&inbounds).Error; err != nil {
				return err
			}
		}
		validInboundIDs := make([]uint, 0, len(inbounds))
		serverIDs := make([]uint, 0, len(inbounds))
		for _, inbound := range inbounds {
			validInboundIDs = append(validInboundIDs, inbound.ID)
			serverIDs = append(serverIDs, inbound.ServerID)
		}
		users[i].InboundIDs = normalizedIDs(validInboundIDs)
		users[i].ServerIDs = normalizedIDs(serverIDs)
		if err := tx.Model(&users[i]).Select("ServerIDs", "InboundIDs", "UpdatedAt").Updates(&users[i]).Error; err != nil {
			return err
		}
	}
	return nil
