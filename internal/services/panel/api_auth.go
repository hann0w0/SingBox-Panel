...
func (a *App) handleLogin(c *gin.Context) {
	var req loginReq
	if !bindJSONLimit(c, &req, maxLoginJSONBytes) {
		return
	}
	...
	if _, normalized, err := validateUsername(account); err != nil {
	...
	ip := clientIP(c)
	ipKey := "ip:" + ip
	accountSum := sha256.Sum256([]byte(account))
	accountKey := hex.EncodeToString(accountSum[:])
	pairKey := "ip:" + ip + "|acct:" + accountKey
	acctKey := "acct:" + accountKey
	...
	var u model.User
	if err := a.db.Where("email_normalized = ? OR (email_normalized IS NULL AND LOWER(email) = ?)", account, account).First(&u).Error; err != nil {
