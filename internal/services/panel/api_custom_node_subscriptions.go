...
func (a *App) deleteCustomNodeSubscription(c *gin.Context) {
	...
	err := a.db.Transaction(func(tx *gorm.DB) error {
		...
		var nodeIDs []uint
		if err := tx.Model(&model.CustomNode{}).Where("subscription_id = ?", id).Pluck("id", &nodeIDs).Error; err != nil {
			return err
		}
		if err := tx.Where("subscription_id = ?", id).Delete(&model.CustomNode{}).Error; err != nil {
			return err
		}
		if err := deleteUserNodeOrderRefs(tx, userNodeTypeCustom, nodeIDs); err != nil {
			return err
		}
