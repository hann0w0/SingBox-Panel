...
func (a *App) listCustomNodes(c *gin.Context) {
	...
	for i := range nodes {
		...
		if !nodes[i].AllUsers && len(nodes[i].UserIDs) > 0 {
			...
			if err := a.db.Select("email").Where("id IN ?", nodes[i].UserIDs).Find(&users).Error; err != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
				return
			}
			for _, u := range users {
...
func (a *App) createCustomNode(c *gin.Context) {
	...
	if err := a.db.Transaction(func(tx *gorm.DB) error {
		if err := validateCustomNodeAudience(tx, node.UserIDs, node.ExcludedUserIDs); err != nil {
			return err
		}
		return tx.Create(node).Error
	}); err != nil {
		if errors.Is(err, errInvalidCustomNodeAudience) {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
...
func (a *App) updateCustomNode(c *gin.Context) {
	...
	columns := []string{"Link", "Protocol", "Address", "Port", "Params", "Group"}
	...
	audienceProvided := req.AllUsers != nil || req.UserIDs != nil || req.ExcludedUserIDs != nil
	...
	if audienceProvided {
		...
		if err := validateCustomNodeAudience(a.db, node.UserIDs, node.ExcludedUserIDs); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		columns = append(columns, "AllUsers", "UserIDs", "ExcludedUserIDs")
	}
	if req.Enabled != nil {
	...
	if req.SortOrder != nil {
		node.SortOrder = *req.SortOrder
		columns = append(columns, "SortOrder")
	}
	if err := a.db.Model(&model.CustomNode{}).Where("id = ?", node.ID).Select(columns).Updates(&node).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	if err := a.db.First(&node, node.ID).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"node": node})
...
var errInvalidCustomNodeAudience = errors.New("用户列表包含不存在的用户")

func validateCustomNodeAudience(db *gorm.DB, userIDs, excludedUserIDs []uint) error {
	ids := normalizedIDs(append(append([]uint{}, userIDs...), excludedUserIDs...))
	if len(ids) == 0 {
		return nil
	}
	var count int64
	...
	if count != int64(len(ids)) {
		return errInvalidCustomNodeAudience
	}
	return nil
}
...
func (a *App) deleteCustomNode(c *gin.Context) {
	...
	err := a.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := deleteUserNodeOrderRefs(tx, userNodeTypeCustom, []uint{node.ID}); err != nil {
			return err
		}
...
func (a *App) batchDeleteCustomNodes(c *gin.Context) {
	...
	var deleted int64
	if err := a.db.Transaction(func(tx *gorm.DB) error {
		result := tx.Delete(&model.CustomNode{}, ids)
		if result.Error != nil {
			return result.Error
		}
		deleted = result.RowsAffected
		return deleteUserNodeOrderRefs(tx, userNodeTypeCustom, ids)
	}); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
