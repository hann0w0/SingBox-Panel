...
// H2: createUser must reject unknown server and inbound IDs before creating
// the account, matching the validation used by updateUser.
func TestCreateUserRejectsUnknownNodeIDs(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, tc := range []struct {
		name string
		body string
	}{
		{name: "unknown server", body: `{"server_ids":[999999]}`},
		{name: "unknown inbound", body: `{"inbound_ids":[999999]}`},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			body := `{"email":"new-` + tc.name + `","password":"test-password",` + tc.body[1:]
			...
			if count != 0 {
				t.Fatalf("invalid request created %d users", count)
			}
		})
	}
}
...
func TestPasswordUpdateRejectsStaleConcurrentSnapshot(t *testing.T) {
	db := testDB(t)
	user := model.User{
		Email: "password-race", Password: "old-hash", Role: model.RoleUser,
		Enabled: true, SubToken: "password-race-sub", ProxyToken: "password-race-proxy", TokenVersion: 3,
	}
	if err := db.Create(&user).Error; err != nil {
		t.Fatal(err)
	}
	stale := user
	updated, err := updatePasswordIfCurrent(db, &user, "new-hash-a")
	if err != nil || !updated {
		t.Fatalf("first password update = %v, %v", updated, err)
	}
	updated, err = updatePasswordIfCurrent(db, &stale, "new-hash-b")
	if err != nil {
		t.Fatal(err)
	}
	if updated {
		t.Fatal("stale concurrent password update unexpectedly succeeded")
	}
	var stored model.User
	if err := db.First(&stored, user.ID).Error; err != nil {
		t.Fatal(err)
	}
	if stored.Password != "new-hash-a" || stored.TokenVersion != 4 {
		t.Fatalf("stored password state = hash %q version %d", stored.Password, stored.TokenVersion)
	}
}
