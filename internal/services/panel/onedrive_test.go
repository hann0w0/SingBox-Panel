...
func TestOneDriveUploadRejectsIncompleteFinalChunk(t *testing.T) {
	oldOAuthURL, oldGraphURL := oneDriveOAuthBaseURL, oneDriveGraphBaseURL
	defer func() {
		oneDriveOAuthBaseURL = oldOAuthURL
		oneDriveGraphBaseURL = oldGraphURL
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/upload":
			...
			_, _ = w.Write([]byte(`{"nextExpectedRanges":["24-"]}`))
		default:
			http.NotFound(w, r)
		}
	}))
	...
	archivePath := filepath.Join(t.TempDir(), "singbox-panel-backup-test.tar.gz")
	if err := os.WriteFile(archivePath, []byte("data-only-backup-archive"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := a.uploadOneDriveBackup(archivePath); err == nil || !strings.Contains(err.Error(), "意外状态") {
		t.Fatalf("upload error = %v, want incomplete final chunk rejection", err)
	}
}
...
func TestOneDriveBackupListRejectsCrossOriginPagination(t *testing.T) {
	oldOAuthURL, oldGraphURL := oneDriveOAuthBaseURL, oneDriveGraphBaseURL
	defer func() {
		oneDriveOAuthBaseURL = oldOAuthURL
		oneDriveGraphBaseURL = oldGraphURL
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/me/drive/items/folder-id/children":
			_ = json.NewEncoder(w).Encode(map[string]any{
				"value":           []oneDriveFile{},
				"@odata.nextLink": "https://attacker.invalid/steal-access-token",
			})
		default:
			http.NotFound(w, r)
		}
	...
	if _, err := a.listOneDriveBackups(); err == nil || !strings.Contains(err.Error(), "不受信任") {
		t.Fatalf("unexpected pagination validation error: %v", err)
	}
}
...
func TestOneDriveRestoreDownloadsDirectlyAndPreservesAuthorization(t *testing.T) {
	...
	restoredApp := &App{cfg: config.PanelConfig{JWTSecret: testRestoredJWTSecret}, db: restored}
