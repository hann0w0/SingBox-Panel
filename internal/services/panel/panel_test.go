...
func TestImportSyncPreservesInboundGrantsByTag(t *testing.T) {
  ...
  "route": {"final":"direct"}
	...
	nodes, err := app.userNodeDetails(&user)
	if err != nil {
		t.Fatal(err)
	}
...
func TestExpiredAdministratorSessionIsRejected(t *testing.T) {
	...
	u := model.User{Email: "admin", Password: "hash", Role: model.RoleAdmin, Enabled: true, ExpireAt: &expires, SubToken: "sub"}
	if err := db.Create(&u).Error; err != nil {
		t.Fatal(err)
	}
	auth := NewAuth("0123456789abcdef0123456789abcdef", db)
	...
	if err := db.Model(&u).Update("expire_at", expired).Error; err != nil {
		t.Fatal(err)
	}
	req := httptest.NewRequest(http.MethodGet, "/ok", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	...
	if w.Code != http.StatusUnauthorized {
		t.Fatalf("expired admin token status = %d", w.Code)
	}
}
...
func TestInstallCommandQuotesEveryArgument(t *testing.T) {
	...
	code := "code'one-time"
	command := installCommand(baseURL, code)
	for _, value := range []string{baseURL + "/api/agent/install.sh", baseURL, code} {
...
func TestDeleteInboundDoesNotExpandExplicitUserAccess(t *testing.T) {
	db := testDB(t)
	server := model.Server{Name: "node", AgentToken: "delete-inbound-token"}
	if err := db.Create(&server).Error; err != nil {
		t.Fatal(err)
	}
	removed := model.Inbound{ServerID: server.ID, Tag: "removed", Type: model.InboundVLESS, ListenPort: 443}
	kept := model.Inbound{ServerID: server.ID, Tag: "kept", Type: model.InboundVLESS, ListenPort: 8443}
	if err := db.Create(&removed).Error; err != nil {
		t.Fatal(err)
	}
	...
	explicitOnlyRemoved := model.User{
		Email: "removed-only", Password: "hash", Role: model.RoleUser, Enabled: true, SubToken: "removed-only-sub",
		ServerIDs: []uint{server.ID}, InboundIDs: []uint{removed.ID},
	}
	explicitBoth := model.User{
		Email: "both", Password: "hash", Role: model.RoleUser, Enabled: true, SubToken: "both-sub",
		ServerIDs: []uint{server.ID}, InboundIDs: []uint{removed.ID, kept.ID},
	}
	wildcard := model.User{
		Email: "wildcard", Password: "hash", Role: model.RoleUser, Enabled: true, SubToken: "wildcard-sub",
		ServerIDs: []uint{server.ID}, InboundIDs: []uint{},
	}
	for _, user := range []*model.User{&explicitOnlyRemoved, &explicitBoth, &wildcard} {
		if err := db.Create(user).Error; err != nil {
			t.Fatal(err)
		}
	}
	if err := db.Transaction(func(tx *gorm.DB) error {
		...
		return removeInboundUserAssignments(tx, removed.ID)
	}); err != nil {
		t.Fatal(err)
	}
	for _, user := range []*model.User{&explicitOnlyRemoved, &explicitBoth, &wildcard} {
	...
	if len(explicitOnlyRemoved.ServerIDs) != 0 || len(explicitOnlyRemoved.InboundIDs) != 0 {
		t.Fatalf("removed-only access expanded: servers=%v inbounds=%v", explicitOnlyRemoved.ServerIDs, explicitOnlyRemoved.InboundIDs)
	}
	if fmt.Sprint(explicitBoth.ServerIDs) != fmt.Sprint([]uint{server.ID}) || fmt.Sprint(explicitBoth.InboundIDs) != fmt.Sprint([]uint{kept.ID}) {
		t.Fatalf("remaining explicit access changed: servers=%v inbounds=%v", explicitBoth.ServerIDs, explicitBoth.InboundIDs)
	}
	if fmt.Sprint(wildcard.ServerIDs) != fmt.Sprint([]uint{server.ID}) || len(wildcard.InboundIDs) != 0 {
		t.Fatalf("whole-server access changed: servers=%v inbounds=%v", wildcard.ServerIDs, wildcard.InboundIDs)
	}
}

func TestDeleteServerWaitsForServerOperation(t *testing.T) {
	gin.SetMode(gin.TestMode)
	db := testDB(t)
	server := model.Server{Name: "node", AgentToken: "serialized-delete-token"}
	if err := db.Create(&server).Error; err != nil {
		t.Fatal(err)
	}
	a := NewApp(config.PanelConfig{JWTSecret: testLiveJWTSecret}, db)
	router := gin.New()
	router.DELETE("/servers/:id", a.deleteServer)

	unlock := a.lockServerOperation(server.ID)
	done := make(chan *httptest.ResponseRecorder, 1)
	go func() {
		recorder := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodDelete, "/servers/"+strconv.FormatUint(uint64(server.ID), 10), nil)
		router.ServeHTTP(recorder, req)
		done <- recorder
	}()
	select {
	case <-done:
		t.Fatal("server deletion did not wait for the active server operation")
	...
	case <-time.After(3 * time.Second):
		t.Fatal("server deletion remained blocked after the operation completed")
	}
}
