...
import { AssignModal, CustomNodesPanel } from '../access/Access'
...
export default function Users() {
  ...
  const mobileUserList = users.length === 0 ? (
    ...
    <VirtualList
      className={`virtual-list mobile-admin-list mobile-user-list${users.length > MAX_VISIBLE_USERS ? ' is-scrollable' : ''}`}
      items={users}
      getKey={userKey}
        ...
        <div className="users-page-grid">
          ...
          >
            ...
            {isMobile ? mobileUserList : <Table
              ...
              dataSource={users}
              pagination={false}
              virtual={users.length > MAX_VISIBLE_USERS}
              scroll={{
                x: 650,
          ...
          <CustomNodesPanel nodes={nodes} loading={nodesLoading} error={nodesError} onNodesChange={loadNodes} />
        </div>
      </div>
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item
            ...
            rules={editing ? [{ min: 8, max: 72 }] : [{ required: true, min: 8, max: 72 }]}
          >
            <Input.Password />
          </Form.Item>
