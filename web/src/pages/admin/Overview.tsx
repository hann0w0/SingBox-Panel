...
export default function Overview() {
  ...
  useEffect(() => {
    const load = () => getOverview().then(setD).catch(() => {})
    load()
      ...
      <Col xs={24}>
        ...
        >
          <Table
            ...
            onRow={(n) => ({ onClick: () => nav(`/admin/servers/${n.id}`), style: { cursor: 'pointer' } })}
            columns={[
              {
                title: '节点',
                render: (_, n: NodeBrief) => (
                  <span>
                    {n.name}
                    {n.region ? <span style={{ color: 'rgba(0,0,0,0.45)' }}> · {n.region}</span> : null}
                  </span>
                ),
              },
              {
