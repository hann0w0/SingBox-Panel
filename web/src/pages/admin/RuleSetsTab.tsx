...
export const RuleSetsTab: React.FC<RuleSetsTabProps> = ({ serverId, ruleSets, onRefresh, rawMode }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingRS) {
        await updateRuleSet(serverId, editingRS.id, payload)
        message.success('规则集更新成功')
      } else {
        await createRuleSet(serverId, payload)
        message.success('规则集添加成功')
      }
      setModalOpen(false)
      onRefresh()
    } catch (err: any) {
  ...
  const handleDelete = async (rs: RuleSet) => {
    try {
      await deleteRuleSet(serverId, rs.id)
